...
namespace Enclave.UdpPerf
{
    public static class UdpSocketExtensions
    {
        ...
        public static async ValueTask<int> SendToAsync(this Socket socket, EndPoint destination, ReadOnlyMemory<byte> data, CancellationToken cancelToken = default)
        {
            return await socket.SendToAsync(data, SocketFlags.None, destination, cancelToken);
        ...
        /// <returns>The number of bytes transferred.</returns>
        public static async ValueTask<SocketReceiveFromResult> ReceiveFromAsync(this Socket socket, Memory<byte> buffer, CancellationToken cancelToken = default)
        {
            return await socket.ReceiveFromAsync(buffer, SocketFlags.None, _blankEndpoint, cancelToken);
